 ...
 */
bool Adafruit_MPR121::begin(uint8_t i2caddr, TwoWire *theWire, uint8_t touchThreshold, uint8_t releaseThreshold) {
  ...
  writeRegister(MPR121_MHDF, 0x01); // Determines the largest magnitude of variation to pass through the baseline filter
  writeRegister(MPR121_NHDF, 0x25); // Determines the incremental change when non-noise drift is detected
  writeRegister(MPR121_NCLF, 0xF0); // Determines the number of samples consecutively greater than the Max Half Delta
                                    // necessary before it can be determined that it is non-noise
  writeRegister(MPR121_FDLF, 0x20); // Determines the rate of operation of the filter. A larger number makes it operate
                                    // slower
  ...
  writeRegister(MPR121_DEBOUNCE, 0x11);
